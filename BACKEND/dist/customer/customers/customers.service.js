...
let CustomersService = class CustomersService {
    ...
    async findByUsername(username) {
        const customer = await this.repo.findOne({ where: { username } });
        if (!customer)
            throw new common_1.NotFoundException('Customer not found');
        return customer;
    }
    async updateByUsername(username, dto) {
        const customer = await this.findByUsername(username);
        Object.assign(customer, dto);
        return await this.repo.save(customer);
    }
    async partialPatch(username, dto) {
        const customer = await this.findByUsername(username);
        Object.assign(customer, dto);
        return await this.repo.save(customer);
    }
    async changeStatus(username, isActive) {
        const customer = await this.findByUsername(username);
